...
test("service worker cache remains readable offline", async ({ context, page }) => {
  ...
  const cached = await page.evaluate(async () => {
    ...
    const assets = ["./", "./styles.css?v=14", "./app.js?v=14", "./feedback-store.mjs?v=14", "./feedback-registry.mjs?v=14", "./recommender.mjs?v=14"];
    return Promise.all(
...
test("the full module graph is cache-versioned for atomic upgrades", async ({ page }) => {
  ...
  const imports = await page.evaluate(async () => {
    const files = ["./app.js?v=14", "./feedback-store.mjs?v=14", "./recommender.mjs?v=14"];
